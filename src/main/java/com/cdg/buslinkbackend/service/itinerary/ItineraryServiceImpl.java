...
public class ItineraryServiceImpl implements ItineraryService {
    ...
    @Autowired
    public ItineraryServiceImpl(ItineraryRepository itineraryRepository, BusRepository busRepository, FrequencyRespository frequencyRespository, ResponseBuilder responseBuilder) {
    ...
    public ResponseEntity<ApiResponse> save(ItineraryRequestDTO itineraryRequestDTO) {
        Bus bus = busRepository.findById(itineraryRequestDTO.getBus_id()).orElseThrow( ()-> new BusNotFoundException(itineraryRequestDTO.getBus_id()));

        if(itineraryRequestDTO.getFrequencies().isEmpty()){
            return responseBuilder.buildResponse(HttpStatus.BAD_REQUEST.value(), "No se han enviado las frecuencias a asignar");
        }

        List<Itinerary> itineraries = itineraryRequestDTO.getFrequencies().stream().map(
                itineraryFrequencyDetailDTO -> {
                    Frequency frequency = frequencyRespository.findById(itineraryFrequencyDetailDTO.getFrequency_id()).orElseThrow( ()-> new FrequencyNotFoundException(itineraryFrequencyDetailDTO.getFrequency_id()));
                    Optional<Itinerary> itineraryOptional = itineraryRepository.findByBusAndFrequencyAndDepartureTime(bus,frequency, itineraryFrequencyDetailDTO.getDepartureTime());
                    return itineraryOptional.orElseGet(() -> new Itinerary(bus, frequency, itineraryFrequencyDetailDTO.getDepartureTime()));
                }
        ).toList();

        return responseBuilder.buildResponse(HttpStatus.CREATED.value(), "Itinerario creado exitosamente", itineraryRepository.saveAll(itineraries));
    }
    ...
    public ResponseEntity<ApiResponse> findByOrigenOrDestiny(String origen, String destiny) {
        ...
        if(Objects.nonNull(origen) && Objects.isNull(destiny)){
            filter =  itineraries.stream().filter(
                    itinerary -> itinerary.getFrequency().getOrigen().equals(origen)
            ).toList();
